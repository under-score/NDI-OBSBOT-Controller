...
def send_ndi_ptz_command(command, value):
    """Send PTZ command to the NDI camera."""
    ...
    try:
        if command == "pan_tilt_speed":
            ...
            ndi.recv_ptz_zoom_speed(ndi_recv, value)
            logging.info(f"zoom to {value}")
        elif command == "home":
...
<div>
    ...
    <br>
    <button id="zoom-in" onmousedown="sendPTZCommand('zoom_speed', 0.05)" onmouseup="sendPTZCommand('zoom_speed', 0)">Zoom+</button>    
    ...
    <button id="zoom-out" onmousedown="sendPTZCommand('zoom_speed', -0.05)" onmouseup="sendPTZCommand('zoom_speed', 0)">Zoom-</button>
</div>
<div>
    &nbsp;
</div>
